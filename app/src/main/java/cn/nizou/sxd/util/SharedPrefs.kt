...
object SimianV2AutomationPrefs {
    ...
    const val RETRY_MAX = "simianv2_retry_max"
    const val RETRY_DELAY = "simianv2_retry_delay"
    const val QUICK_SUBMIT = "simianv2_quick_submit"
    const val QUICK_DELAY = "simianv2_quick_delay"
    ...
    val autoAnswerDelay get() = modulePrefs.getString(AUTO_ANSWER_DELAY, "9500")?.toLongOrNull()?.coerceAtLeast(0L) ?: 9500L
    val submitInterval get() = modulePrefs.getString(SUBMIT_INTERVAL, "20")?.toLongOrNull()?.coerceAtLeast(0L) ?: 20L
    val retryMax get() = modulePrefs.getInt(RETRY_MAX, 10)
    val retryDelay get() = modulePrefs.getString(RETRY_DELAY, "20")?.toLongOrNull()?.coerceAtLeast(0L) ?: 20L
    val quickSubmit get() = modulePrefs.getBoolean(QUICK_SUBMIT, false)
    val quickDelay get() = modulePrefs.getString(QUICK_DELAY, "600")?.toLongOrNull()?.coerceAtLeast(0L) ?: 600L
