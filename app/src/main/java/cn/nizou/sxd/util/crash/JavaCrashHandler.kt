...
/**
 * Java 崩溃处理器
 * 捕获未捕获异常 → 收集崩溃信息 → 保存崩溃日志 → 委托默认 handler
 *
 * 移植自 wekit utils/crash/JavaCrashHandler.kt；NativeCrashHandler 不移植。
 * 差异点：
 * - getThreadId 直接用 `thread.id`
 * - context 用反射取宿主 Application（对应 wekit HostInfo.application），失败时降级
 * - 内部日志走 t1 的 [WeLogger]，terminate 前 [WeLogger.flush] 保证异步日志落盘
 */
object JavaCrashHandler : Thread.UncaughtExceptionHandler {
    ...
    private fun collectAndSave(thread: Thread, throwable: Throwable) {
        ...
        // 收集崩溃信息（宿主进程早期崩溃时 Application 可能为 null，收集器内部会降级）
        val context = runCatching { currentApplication() }.getOrNull()
        val crashInfo = CrashInfoCollector.collectCrashInfo(context, throwable, "JAVA")
